...
#define BIT32_IS_CONTINUOUS(mask) (BIT32_FLS(mask) - BIT32_FFS(mask) + 1 == BIT32_CNT(mask))
...
#define REG32_TRIGGER_SC(reg, mask) \
  do                                \
  {                                 \
    REG32(reg) |= (mask);           \
    REG32(reg) &= ~(mask);          \
...
#define REG32_TRIGGER_CS(reg, mask) \
  do                                \
  {                                 \
    REG32(reg) &= ~(mask);          \
    REG32(reg) |= (mask);           \
...
#define ntohs(x) (0xff00 & x << 8) | (0x00ff & x >> 8)
