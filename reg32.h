...
#define REG32_FIELD_GET_BITS(reg, x, n) ((int)REG32_GET((reg), MASK32((x), (n))) >> (x))
#define REG32_FIELD_SET_BITS(reg, x, n, val)                                             \
  do {                                                                                   \
    REG32(reg) = (uint32_t)(REG32(reg) & (~MASK32((x), (n))) | (((val) & MASK(0, (n))) << (x))); \
  } while (0)
#define REG32_FIELD_GET(reg, mask)      ((int)REG32_GET((reg), (mask)) >> BIT32_FFS(mask))
...
#define REG32_BYTE_GET(reg, byte)    REG32_GET((reg), MASK32((byte) * CHAR_BIT, CHAR_BIT))
#define REG32_BYTE_SET(reg, byte)    REG32_SET((reg), MASK32((byte) * CHAR_BIT, CHAR_BIT))
#define REG32_BYTE_CLEAR(reg, byte)  REG32_CLEAR((reg), MASK32((byte) * CHAR_BIT, CHAR_BIT))
#define REG32_BYTE_TOGGLE(reg, byte) REG32_TOGGLE((reg), MASK32((byte) * CHAR_BIT, CHAR_BIT))
#define REG32_BYTE_BOOL(reg, byte)   REG32_BOOL((reg), MASK32((byte) * CHAR_BIT, CHAR_BIT))
